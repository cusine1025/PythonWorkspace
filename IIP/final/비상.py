    ...
    def process(self, t, frontImage, rearImage, frontLidar, rearLidar):
                        ...
                        if type(empty[i_5]) == tuple:
                            ...
                            if i_5 == 0:
                                ...
                                for m in range(1, a+1):
                                    empty.insert(i_5 - m +1, before + gap*m)
       ...
       # (1) 1번 삼거리 
        if R.count("없음") >= 11 :
            bunmo= V[20] // 10
            velocity = 1000 // bunmo
            if 240 <= V[20] <=255 and R.count("없음") <= 16: steer = 35 + 700 // bunmo + standard_steer
            elif (Idx_R >= 5 and  V[20] > 200): steer = 30 + 700 // bunmo +standard_steer
        if Idx_R >= 15: steer = 60 + standard_steer      #elif 구문 2개 없어도 되지 않을까?
       # (2) T자
        if L.count("없음") >= 11 and R.count("없음") >= 11:
            steer = 45 + 60//bunmo + standard_steer
            velocity =1000// bunmo
       ...
       # 6. 속도 조정 코드
        ...
        # print ('total_1=', total_1, 'average=', average, 'steer=', average//5)
        print (empty)
        print (R)
