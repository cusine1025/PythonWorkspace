       ...
       # (1) 1번 삼거리 
        if  L_index + 1 != R_index:
            if R.count("없음") >= 10 :
                velocity = 80
                bunmo= V[20] // 10
                if V[20] == 255 and R.count("없음") <= 16: steer = 40 + 600 // bunmo + standard_steer
                elif Idx_R >= 155: steer = 40 + 600 // bunmo +standard_steer
                elif Idx_R >= 3 and V[20] < 200: steer = 40 + 600 //bunmo + standard_steer      #elif 구문 2개 없어도 되지 않을까?

       # (2) T자
        if L.count("없음") >= 14 and R.count("없음") >= 14:
            steer = 30 + 500//bunmo + standard_steer
            velocity = 40
            think_t = steer
