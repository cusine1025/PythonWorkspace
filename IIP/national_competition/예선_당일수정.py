       ...
       # (1) 1번 삼거리 
        if R.count("없음") >= 14 :
            ...
            if V.count(255) >= 4 and R.count("없음") <= 16: 
                velocity = 40 
                steer = 40 + 300 // bunmo + standard_steer
                status = "삼거리"
            elif (Idx_R >= 5 and  V[20] > 200): steer = 60 + 400 // bunmo +standard_steer
            elif Idx_R >= 15: steer = 60 + 400 //bunmo + standard_steer

       # (2) T자
        if L.count("없음") >= 10 and  R.count("없음") >= 13:     #만약 딜레이가 심한 환경이라면 18> 없애주기
            bunmo= V[20] // 10
            if 100 < V[20] < 190:
                steer = 50 + 400//bunmo + standard_steer
                velocity = 40
                status = "T자"

       # 6. 속도 조정 코드
        if -15 + standard_steer < steer < 15 + standard_steer :
            velocity = 120
            status = "속도조정"
