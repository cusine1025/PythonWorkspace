    ...
    def process(self, t, frontImage, rearImage, frontLidar, rearLidar):
        ...
        for i_1 in range (0,21) :
            ...
            if t < 20-i_1 :
                ...
                if R[i_1]==321: R[i_1] = "없음"
                if L[i_1] ==320: L[i_1] = "없음"
       ...
       # (1) 1번 삼거리 
        if R.count("없음") >= 12 :
            velocity = 45
            bunmo= V[20] // 50
            if 240 <= V[20] <=255 and R.count("없음") <= 16: steer = 25 + 70 // bunmo + standard_steer
            elif (Idx_R >= 5 and  V[20] > 200): steer = 25 + 70 // bunmo +standard_steer
            elif Idx_R >= 15: steer = 25 + 80 //bunmo + standard_steer      #elif 구문 2개 없어도 되지 않을까?
       # (2) T자
        if L.count("없음") >= 14 and R.count("없음") >= 14:
            steer = 15 + 60//bunmo + standard_steer
            velocity = 45
