    ...
    def process(self, t, frontImage, rearImage, frontLidar, rearLidar):
       ...
       # (1) 1번 삼거리 
        if R.count("없음") >= 12 :
            velocity = 90
            bunmo= V[20] // 10
            if 240 <= V[20] <=255 and R.count("없음") <= 16: steer =  20 + 250 // bunmo + standard_steer
            elif (Idx_R >= 5 and  V[20] > 200): steer = 20 + 250 // bunmo +standard_steer
            elif Idx_R >= 15: steer = 20 + 250//bunmo + standard_steer      #elif 구문 2개 없어도 되지 않을까?
       # (2) T자
        if L.count("없음") >= 17 and R.count("없음") >= 17:
            steer = 15 + 100//bunmo + standard_steer
            velocity = 80
