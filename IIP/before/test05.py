...
class Planning(BasePlanning):
    ...
    def process(self, t, frontImage, rearImage, frontLidar, rearLidar):
        ...
        if V[4] < 60:
            steer = -80 +5 #원래 80인데 너무 일찍 돌아서 꼬불꼬불에서 차선이탈함 => 60으로 조정

        elif V[5] < 30: # 조정 필요 V값 차이
            steer = - (120 - V[5]) +5
        ...
        if V[3] < 180 :
            if L[1] == 320 and R[1] == 321:
                ...
                change_right = V[4] - V[3]
                change_left = V[2] - V[3]

                if -1 <= change_right - (V[5] - V[4]) <= 1 or -1 <= change_left - (V[1] - V[2]) <= 1: 
                    ...
                    if L[1] < 320 and R[1] < 321:
                        if change_right > 0 :
                            steer = 100 // change_right
                        if change_right < 0 :
                            steer = -100 // change_right
                            
        
           #대회 전에 기본 STEER 값 찾아야 함
