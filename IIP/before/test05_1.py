...
class Planning(BasePlanning):
    ...
    def process(self, t, frontImage, rearImage, frontLidar, rearLidar):
        ...
        if R[2]-L[2] > 20 :
            steer = (R[2] - L[2]) // 6 -25  #매우 좋음


        if V[4] < 45:
            steer = -80 -25 #원래 80인데 너무 일찍 돌아서 꼬불꼬불에서 차선이탈함 => 60으로 조정

        elif V[5] < 20: # 조정 필요 V값 차이
            steer = - (120 - V[5]) -25
